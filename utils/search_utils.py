...
def build_results(
    ...
    for td in titles_map.values():
        ...
        out_count = sum(1 for s in volume_best.values() if s not in ("Available", "On Hold"))
        ...
        for linfo in sorted(td["lib_data"].values(), key=lambda x: x["library_id"]):
            ...
            for vol_num in sorted(linfo["volumes"].keys()):
                ...
                for bid in sorted(
                    branch_statuses.keys(), key=lambda b: linfo["branch_names"].get(b, "")
                ):
                    ...
                    if cur_best is None or STATUS_PRIORITY[status] > STATUS_PRIORITY.get(
                        cur_best, -1
                    ):
            ...
            branch_list = [
                ...
                for bid in sorted(
                    branch_best.keys(), key=lambda b: linfo["branch_names"].get(b, "")
                )
            ]
